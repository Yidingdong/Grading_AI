    ...
    if not enrollments:
        ...
        for enr_loop in enrollments:
            ...
            student_ui_submissions = list(files_metadata_collection.find(
                {"course_id": course_obj.course_id, "student_id": student_for_submission_view.id,
                 "file_type": "student_answer_sheet"}).sort("upload_timestamp", -1))
...
if not st.session_state.logged_in:
    ...
    if st.session_state.role == "Teacher":
        ...
        try:
            ...
            st.sidebar.error(
                f"DB error loading student courses: {e}"); student_active_course_pages, student_completed_course_pages = [], []
